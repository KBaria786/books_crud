...
public class PublisherController {
    ...
    @Operation(summary = "Find all publishers with pagination")
    @ApiResponses(value =
            {@ApiResponse(responseCode = "200", description = "successful", content = @Content(array = @ArraySchema(schema = @Schema(implementation = Publisher.class)))),
                    @ApiResponse(responseCode = "500", description = "internal server error", content = @Content)})
    @GetMapping()
    public ResponseEntity<List<Publisher>> findAll(@Parameter(name = "page", description = "page")
        ...
        httpHeaders.add("X-Total-Count", publisherService.getTotalCount().toString());
        return ResponseEntity.ok().body(publisherService.findAll(page, limit));
    }
